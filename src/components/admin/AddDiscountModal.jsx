...
const AddDiscountModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleDiscountChange = (e) => {
    ...
    // Only set the discount if it's not negative
  ...
  return (
    ...
    >
      <div className="bg-white rounded-lg shadow-lg p-6 w-96 relative">
        ...
        <h2 className="text-lg font-semibold mb-4">Add Discount to Product</h2>
        ...
        {selectedProduct && (
          <>
            <label htmlFor="discount" className="block mt-4 mb-2">Enter Discount Percentage:</label>
            <input
              type="number"
              id="discount"
              className="w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={discount}
              onChange={handleDiscountChange} // Use the new handler here
              min="0"
              max="100"
        ...
        <button
          ...
          onClick={() => onSubmit({ productId: selectedProduct, discount })}
          disabled={!selectedProduct || discount === ''} // Disable if no product is selected or discount is empty
        >
          Add Discount
        </button>
      </div>
